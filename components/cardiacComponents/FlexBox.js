  ...
  return (
    <div
      className={`grid gap-6 mb-12 ${
        count === 3 ? 'md:grid-cols-2' : count === 1 ? 'md:grid-cols-1' : 'md:grid-cols-2'
      }`}
    >
      {data.sections.map((section, index) => {
        const bgColor = index < 2 ? '#D84397' : '#0e56a0'
        const isThirdFullWidth = count === 3 && index === 2

        return (
          <div
            key={index}
            style={{ backgroundColor: bgColor }}
            className={`text-white p-6 rounded ${isThirdFullWidth ? 'md:col-span-2' : ''}`}
          >
