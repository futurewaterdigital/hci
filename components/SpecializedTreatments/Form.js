...
function ConsultantForm() {
    ...
    const handleTextChange = (e) => {
        ...
        if (name === "age") {
            if (!ageRegex.test(value) || parseInt(value) < 1 || parseInt(value) > 120) {
                setErrors({ [name]: ageErrors.message });
            } else {
                setErrors({});
                setAge(value);
            }
        ...
        if (name === "city") {
            if (!cityRegex.test(value)) {
                setErrors({ [name]: cityErrors.message });
            } else {
                setErrors({});
                setCity(value);
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(
                "https://cdn.healthcareinternational.in/wp-json/contact-form-7/v1/contact-forms/415/feedback",
                {
                    method: "POST",
                    body: formData,
                    credentials: "include",
